...
@ratelimit(key='user', rate='1/h', block=True)
@api_view(["POST"])
@permission_classes([IsAuthenticated])
...
def lectures(request, course_id):
    ...
    #     if not enrolled_courses.filter(id=course_id).exists():
    ...
    for lecture in lectures:
        data.append({
            ...
            "meeting_link": lecture.meeting_link if lecture.meeting_link else None,
        })
    return Response(data)
